...
def initialize_repo(
    ...
    if not projectDir:
        return {
            "error": "Project directory (fully qualified path) is required"
        }
    ...
    try:
        ...
        clone_cmd = ["git", "clone", "https://github.com/Nuosis/js-ai-dev-environment.git", os.path.basename(target_dir)]
        ...
        npm_cmd = ["npm", "install"]
        npm_process = subprocess.run(npm_cmd, capture_output=True, text=True, check=True)
        
        # Create FileMakerService.js if it doesn't exist
        fm_service_path = os.path.join(target_dir, "src", "services", "FileMakerService.js")
...
""")
        ...
        if 'original_dir' in locals():
            os.chdir(original_dir)
        return {
            "error": f"Command failed: {e.cmd}. Error: {e.stderr}"
        }
        ...
        if 'original_dir' in locals():
            os.chdir(original_dir)
        return {
            "error": f"Failed to initialize widget project: {str(e)}"
        }
...
def get_prompt(
    ...
    scriptName: str = "",
    techStack: list = None,
    useTypeScript: bool = False,
    stateLibrary: str = "",
    stylePaths: list = None
):
    """
    ...
    Args:
        widgetIntention: Description of the widget's intended purpose. Ask: "What is the widget's intended purpose? The more detail you provide the better"
        projectName: Name of the widget project. Use the name of the current directory. Does not require usewr input.
        fileName: FileMaker file name. ASK: "What is the FileMaker file name? Make sure to include .fmp12"
        fileMakerPath: FileMaker file path or URL. ASK: "Where is the FileMaker file located? Provide a url or path."
        scriptName: FileMaker script name to run. ASK: "What is the name of the FileMaker script to run to upload the html code? defaut is 'UploadToHTML'"
        techStack: Tech stack options (1=CommonJS, 2=React, 3=Next.js). ASK: "What is the tech stack? (1=CommonJS, 2=React, 3=Next.js)"
        useTypeScript: Whether to use TypeScript. ASK: "Do you want to use TypeScript? (yes/no)"
        stateLibrary: Name of the state management library. ASK: "What state management library do you want to use? (e.g., Redux, MobX, Zustand, None)"
        stylePaths: List of paths to style/example images. ASK: "What styles/examples do you want to use? Provide paths or URLs to images or example CSS."
    
    Returns:
        A dictionary containing the prompt for the agent.
    """
    ...
    if not widgetIntention:
        return {
            "error": "Widget intention is required"
        }
